...
4. **`mcp-server/` = đường GHI THỨ HAI vào prod DB:** thiếu lớp guard mà web có (WorkspaceMember, PAID-payroll lock, AuditLog, optimistic-version, rank-D). → **Một lớp guard dùng chung** trong mcp-server (xem FIX_QUEUE Đợt 3.5). High = cross-tenant thật; các Low = parity/defense-in-depth (actor MCP = service-account admin).
