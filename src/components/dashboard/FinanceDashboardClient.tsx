        ...
        <div className="max-w-[1200px] mx-auto p-4 space-y-8">
            ...
            <div className="flex flex-col gap-3">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <span className="flex items-center gap-1.5 bg-primary/10 border border-primary/20 text-primary-accent px-3 py-1 rounded-full text-sm">
                    ...
                    <div className="flex items-center bg-zinc-900/80 border border-white/10 rounded-xl p-0.5">
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all duration-200 ${
                                !isUSD ? 'bg-emerald-500/20 text-emerald-400 shadow-sm' : 'text-muted-foreground hover:text-zinc-300'
                            }`}
                        >
                            VNĐ
                        </button>
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all duration-200 ${
                                isUSD ? 'bg-blue-500/20 text-blue-400 shadow-sm' : 'text-muted-foreground hover:text-zinc-300'
                            }`}
                        >
                            USD
                        </button>
                    ...
                    <div className="flex items-center bg-zinc-900/80 border border-white/10 rounded-xl p-0.5">
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all duration-200 flex items-center gap-1.5 ${
                                isTeam ? 'bg-primary/20 text-primary-accent shadow-sm' : 'text-muted-foreground hover:text-zinc-300'
                            }`}
                        >
                        ...
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all duration-200 flex items-center gap-1.5 ${
                                !isTeam ? 'bg-purple-500/20 text-purple-400 shadow-sm' : 'text-muted-foreground hover:text-zinc-300'
                            }`}
                        >
                ...
                <div className={`grid grid-cols-1 ${isTeam ? 'md:grid-cols-3' : 'md:grid-cols-1'} gap-4`}>
                    ...
                    {isTeam && (
                        <div className="bg-zinc-950/50 border border-red-500/20 rounded-2xl p-5 relative overflow-hidden group hover:border-red-500/40 transition-all">
                            ...
                            <p className="text-xs text-muted-foreground mb-3">Thù lao đã chi trả cho nhân viên</p>
            ...
            <div>
                <div className="flex items-center gap-2 mb-3">
                    <div className="w-2 h-2 rounded-full bg-primary shadow-[0_0_8px_rgba(99,102,241,0.6)] animate-pulse" />
                    <span className="text-xs font-bold text-primary-accent uppercase tracking-widest">
                        {isTeam
                ...
                <div className={`grid grid-cols-1 ${isTeam ? 'md:grid-cols-3' : 'md:grid-cols-1'} gap-4`}>
                    ...
                    {/* Projected Revenue */}
                    <div className="bg-zinc-900/30 border border-dashed border-primary/20 rounded-2xl p-5 relative overflow-hidden group hover:border-primary/40 transition-all">
                        ...
                        <div className="text-2xl font-bold text-primary-accent/80 font-mono tabular-nums">
                    ...
                    {isTeam && (
                        <div className="bg-zinc-900/30 border border-dashed border-rose-500/20 rounded-2xl p-5 relative overflow-hidden group hover:border-rose-500/40 transition-all">
                            <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mb-1">DỰ KIẾN CHI PHÍ</p>
                    ...
                    {isTeam && (
                        <div className="bg-zinc-900/30 border border-dashed border-teal-500/20 rounded-2xl p-5 relative overflow-hidden group hover:border-teal-500/40 transition-all">
                            <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mb-1">DỰ KIẾN LỢI NHUẬN</p>
            ...
            ══════════════════════════════════ */}
            {isTeam && (
                <div className="bg-gradient-to-r from-primary/8 via-zinc-900/20 to-emerald-500/8 border border-white/5 rounded-2xl p-5 flex items-center gap-4 flex-wrap shadow-inner">
                    <div>
                        <p className="text-[10px] font-bold text-primary-accent uppercase tracking-widest mb-1">Tổng doanh thu</p>
                        <p className="text-lg font-bold text-primary-accent font-mono tabular-nums">{fmt(data.projectedRevenueVND)}</p>
                    </div>
                    ...
                    <div className="ml-auto text-right pl-4 border-l border-white/5">
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Biên LN</p>
            ...
            <div className="bg-zinc-950/40 border border-white/5 rounded-2xl overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
                    ...
                    <span className="text-[11px] text-muted-foreground bg-zinc-900/50 px-2.5 py-1 rounded-lg border border-white/5">
                ...
                <div className={`grid ${isTeam ? 'grid-cols-[2fr_1fr_1fr_1fr_1fr]' : 'grid-cols-[2fr_1fr_1fr]'} gap-0 px-6 py-2.5 border-b border-white/5 bg-zinc-900/20`}>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Task & Trạng thái</p>
                    {isTeam && <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Người làm</p>}
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest text-right">Doanh thu</p>
                    {isTeam && <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest text-right">Thù lao</p>}
                    {isTeam && <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest text-right">Lợi nhuận ròng</p>}
                </div>
                ...
                <div className="divide-y divide-white/[0.04]">
                    {data.transactions.slice(0, 50).map(t => (
                        ...
                        >
                                ...
                                <div className="flex items-center gap-2">
                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${
                                        t.isCompleted
                                            ...
                                            : 'bg-primary/10 text-primary-accent border-primary/20'
                                    }`}>
                            ...
                            {isTeam && (
                                <p className="text-sm text-muted-foreground">{t.assignee}</p>
                            )}
