...
function renderOverview(o: DigestParams['overview']): string {
    if (!o) return ''
    const cell = (icon: string, label: string, value: number, color: string) =>
        `<td align="center" style="padding:14px 6px;background:#fafafa;border:1px solid ${COLORS.BORDER};border-radius:10px;width:25%;vertical-align:top;">
...
<div style="font-size:11px;color:${COLORS.TEXT_SECONDARY};margin-top:4px;">${label}</div>
</td>`
    return `<div style="margin-top:8px;margin-bottom:6px;">
<div style="font-size:12px;font-weight:700;color:${COLORS.BRAND};text-transform:uppercase;letter-spacing:0.06em;padding:6px 0;border-bottom:2px solid ${COLORS.BRAND}30;margin-bottom:12px;">📊 Tổng quan</div>
<table role="presentation" cellpadding="0" cellspacing="6" border="0" width="100%">
<tr>
${cell('🟢', 'Hoàn thành', o.completedTasksCount, COLORS.SUCCESS)}
${cell('🟡', 'Đang chờ', o.pendingTasksCount, COLORS.WARNING)}
${cell('❗', 'Quá hạn', o.overdueTasksCount, COLORS.DANGER)}
${cell('💬', 'Chưa đọc', o.unreadMessagesCount, COLORS.INFO)}
</tr>
</table>
</div>`
}

export async function digestDaily(params: DigestParams): Promise<RenderedEmail> {
    ...
    const body = `
${heading('📬', `Tóm tắt ngày ${escapeHtml(params.timeRange)}`)}
<div style="color:${COLORS.TEXT_SECONDARY};font-size:13px;margin-bottom:16px;">${escapeHtml(previewLine || 'Không có thông báo mới')}</div>
${renderOverview(params.overview)}
...
${ctaRow([{ text: '🚀 Mở HustlyTasker →', url: homeLink }])}`
    ...
    const html = await wrapTemplate({
        ...
        eventType: 'DIGEST_DAILY',
        appUrl: params.appUrl,
        workspaceId: params.workspaceId,
    })

    return {
        subject: `📬 Tóm tắt ngày ${params.timeRange}: ${params.totalCount} thông báo`,
        html,
    }
}
