...
export default function UserPageTabs({ users, currentUser, profiles, incomingRequests, workspaceId }: { users: any[], currentUser: any, profiles?: any[], incomingRequests?: any[], workspaceId: string }) {
    // Default tab = payroll (feature chính). 'users' tab vẫn còn dạng read-only
    // performance overview. Member management chuyển sang /admin/members.
    const [activeTab, setActiveTab] = useState<'payroll' | 'users' | 'profiles'>('payroll')
