...
| N10 | `wrapTemplate.ts:159` — `avatar()` nội suy `url` THÔ vào `img src`, đang chạy ở email bình luận task và email đổi trạng thái. Người phản biện nói "chỉ một nơi ghi `avatarUrl`" — **tôi kiểm và điều đó SAI: đăng nhập Google cũng ghi** (`google-auth.ts:189`, `:213`). Chưa khai thác được vì `info.picture` do Google trả về, nhưng bất biến thật là "mọi nơi ghi đều từ nguồn tin cậy", yếu hơn nhiều. `profile-actions.ts` còn sẵn dòng `// avatar: data.avatar` bị comment. | ✅ **vá trong `7747cd4`** — `safeImageSrc` (allowlist `^https?://` rồi escape), không đạt thì rơi về ảnh chữ cái; `initial` cũng escape. Kèm hàng rào hồi quy `scripts/assert-email-avatar-escaping.ts` (19/19 đạt, chạy thật với đầu vào thù địch) |
| N11 | `getStoredEnvVars()` (electron) KHÔNG lọc khoá, và `next-server.ts` trải thẳng kết quả vào `env` của tiến trình con → bất kỳ khoá nào lọt vào file store đều thành biến môi trường Node (`NODE_OPTIONS`…). Danh sách cho phép của HT-038 chỉ đối xứng ở chiều GHI. | ✅ **vá trong `b252760`** — hai chiều dùng chung một danh sách. Kiểm bằng máy: danh sách ≡ đúng 8 trường `EnvSchema` nên không biến hợp lệ nào bị loại; `next-server.ts` trải `process.env` TRƯỚC nên PATH/NODE_ENV không chịu ảnh hưởng |
