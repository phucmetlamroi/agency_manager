...
export default function SearchPalette({ deliverables, invoices, onClose, openDeliverable, openInvoice }: {
    ...
    const hits = useMemo<Hit[]>(() => {
        ...
        const dels: Hit[] = deliverables
            .filter(d => !term || d.title.toLowerCase().includes(term) || (d.client?.name || '').toLowerCase().includes(term))
            .slice(0, 8)
            .map(d => ({ kind: 'del', id: d.id, title: d.title, sub: [d.client?.name, d.type].filter(Boolean).join(' · '), status: d.clientStatus }))
        const invs: Hit[] = invoices
            .filter(i => !term || i.invoiceNumber.toLowerCase().includes(term))
            .slice(0, 5)
            .map(i => ({ kind: 'inv', id: i.id, title: i.invoiceNumber, sub: fmtMoney(i.totalDue), status: mapInvoiceStatus(i.status) }))
        return [...dels, ...invs]
        ...
        <>
            ...
            <div role="dialog" aria-modal="true" aria-label="Search" style={{ position: 'fixed', top: '12vh', left: '50%', transform: 'translateX(-50%)', width: 'min(600px, 94vw)', zIndex: 71, background: 'var(--paper-raised)', border: '1px solid var(--hairline-strong)', borderRadius: 10, boxShadow: 'var(--shadow-modal)', overflow: 'hidden' }} className="desk-rise">
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 18px', borderBottom: '1px solid var(--hairline)' }}>
                    ...
                    <input ref={inputRef} value={q} onChange={e => setQ(e.target.value)} onKeyDown={onKey} placeholder="Search productions, statements…" style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontFamily: 'var(--font-body)', fontSize: '1rem', color: 'var(--ink)' }} />
